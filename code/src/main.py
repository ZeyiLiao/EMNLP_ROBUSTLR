...
from generative_model_torch_nn import LRGenerative2
...
def parse_args(args=None):
	...
	parser.add_argument('--part_of_train',      default=1,         type = float, help="Partial of training dataset")
	parser.add_argument('--compute_metrics',    action='store_true')
	parser.add_argument('--filter_unk', 	    action='store_true')
	parser.add_argument('--filter_false', 	    action='store_true')
...
def main(args, splits='all'):
	...
	dm = DataModule(
			...
			part_of_train= args.part_of_train,
			filter_unk=args.filter_unk,
			filter_false=args.filter_false,
		)
	dm.setup(splits=splits)

	print(f'Loading {args.model} - {args.arch} model...')
	if args.model == 'lrdiscriminative' or args.model == 'lrgenerative':
		if args.arch not in ["t5_11b","t5_3b"]:
			...
			model = LRGenerative2(
				...
				num_classes=args.num_classes,
				compute_metrics=args.compute_metrics,
				ckpt_t5_huge_model = args.ckpt_path,
			)


	elif args.model == 'spurious':
		model = model_dict[args.model](
				arch=args.arch,
				train_batch_size=args.train_batch_size,
				eval_batch_size=args.eval_batch_size,
...
if __name__ == '__main__':
	...
	if args.filter_unk or args.filter_false:
		assert args.num_classes == 2, 'Set --num_classes 2 with --filter_unk or --filter_false'
	...
	if not args.evaluate_ckpt:
		...
		if args.hf_name in ["t5-11b","t5-3b"]:
			prediction_list = []
			device = torch.device("cuda")
			model.eval()
			t5_11b_dataloader = dm.test_dataloader()
			model.reasoner.parallelize()
			model.compute_metrics = True
			with torch.no_grad():
				for batch in tqdm(t5_11b_dataloader):
					batch['all_inps'] = batch['all_inps'].to(device)
					prediction_list.append(model.run_step(batch,"test"))
				model.aggregate_epoch(prediction_list,"test")
		else:
